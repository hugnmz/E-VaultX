...
| Software Requirements Specification (SRS) Version 1.1  ·  IEEE 830 / ISO/IEC 29148 Compliant  ·  Java-First Revision |
| :---: |
...
VaultX áp dụng kiến trúc microservices với 6 Java service riêng biệt, mỗi service có database riêng (Database-per-Service pattern). Giao tiếp đồng bộ qua REST được điều phối bởi Nginx reverse proxy (API Gateway layer). Giao tiếp bất đồng bộ qua Apache Kafka (KRaft mode). C++ FX Rate Engine là component biệt lập, giao tiếp với Java consumer qua Kafka topic — không có direct REST call vào Java service. Mỗi Java service áp dụng layered architecture chuẩn: Controller → Service → Repository (Spring Data JPA), với DTO↔Entity mapping qua MapStruct và schema versioning qua Flyway.
...
### **3.3.1 Synchronous Communication (REST via API Gateway)**

Client → Nginx (API Gateway) → Java Service. Nginx xử lý TLS termination, routing theo path prefix (/api/v1/auth → Identity Service, /api/v1/wallets → Wallet Service, ...), và rate limiting bằng ngx\_http\_limit\_req\_module. JWT được validate tại Java service layer (Spring Security filter chain) — Nginx chỉ forward Authorization header, không decode JWT. Circuit breaker pattern được implement bằng Resilience4j standalone (không phụ thuộc Spring Cloud) tại mỗi service khi gọi downstream.
...
| Authentication | Authorization: Bearer {access\_token} cho tất cả protected endpoint |
| Idempotency | Header Idempotency-Key: {uuid} bắt buộc cho POST transfer request |
| Pagination | Query params: ?page=0\&size=20\&sort=created\_at,desc |
| Date/Time | ISO 8601 UTC: 2026-05-11T07:30:00Z |
| Error Format | { error\_code, message, detail, trace\_id } |
| Success Format | { data, meta } trong đó meta chứa pagination info nếu có |
...
| ----- | ----- | ----- | ----- |
| POST | /api/v1/auth/register | None | Đăng ký tài khoản mới |
| POST | /api/v1/auth/login | None | Đăng nhập, nhận JWT |
| POST | /api/v1/auth/refresh | Refresh Cookie | Refresh access token |
| POST | /api/v1/auth/logout | Bearer | Revoke refresh token |
| POST | /api/v1/kyc/submit | Bearer (USER) | Submit KYC documents |
| GET | /api/v1/wallets | Bearer | Liệt kê tất cả ví của user |
| POST | /api/v1/wallets | Bearer | Tạo ví mới cho currency |
| GET | /api/v1/wallets/{id}/transactions | Bearer | Lịch sử giao dịch của ví |
| POST | /api/v1/transfers | Bearer \+ Idempotency-Key | Tạo transfer (P2P hoặc FX) |
| GET | /api/v1/transfers/{id} | Bearer | Xem chi tiết transfer |
| GET | /api/v1/fx/rates | Bearer | Lấy tỷ giá hiện tại từ cache |
| GET | /api/v1/fx/preview | Bearer (PREMIUM) | Preview FX conversion (rate \+ fee) |
| GET | /api/v1/analytics/spending | Bearer | Tổng hợp chi tiêu theo category |
| GET | /api/v1/analytics/timeline | Bearer | Timeline chi tiêu 30/90 ngày |
| GET | /api/v1/analytics/export | Bearer | Export CSV lịch sử giao dịch |
| GET | /api/v1/analytics/budgets | Bearer | Xem budget vs actual |
| PUT | /api/v1/analytics/budgets/{id} | Bearer | Cập nhật budget |
| GET | /api/v1/admin/users | Bearer (ADMIN) | Danh sách user |
| PUT | /api/v1/admin/users/{id}/status | Bearer (ADMIN) | Lock/unlock tài khoản |
| PUT | /api/v1/admin/kyc/{id}/review | Bearer (ADMIN) | Approve/reject KYC |
| GET | /api/v1/admin/fraud-flags | Bearer (ADMIN) | Danh sách flagged transaction |
| PUT | /api/v1/admin/fraud-flags/{id}/resolve | Bearer (ADMIN) | Resolve fraud flag |
| PUT | /api/v1/admin/fx-config | Bearer (ADMIN) | Cập nhật spread config |
...
| ----- | ----- | ----- | ----- |
| 1 | Foundation: Auth \+ Wallet Core | 4–5 tuần | Đăng ký, đăng nhập, xem số dư multi-currency |
| 2 | Transfer \+ Saga \+ Idempotency | 5–6 tuần | Chuyển tiền P2P end-to-end; thử nghiệm rollback khi fail |
| 3 | Analytics \+ CQRS \+ Spring Batch | 4–5 tuần | Dashboard analytics đầy đủ; Spring Batch job chạy ổn định; CQRS read side |
| 4 | C++ FX Rate Engine \+ FX Conversion | 4–5 tuần | Đổi USD → VND với tỷ giá live từ C++ engine; FX conversion end-to-end |
| 5 | Fraud Detection \+ Notification | 3–4 tuần | Trigger fraud, xem alert real-time qua WebSocket |
| 6 | DevOps: K8s \+ Observability \+ CI/CD | 3–4 tuần | Production-like environment; Grafana dashboard; k6 load test |

## **8.2 Phase 1 — Foundation**

### **Deliverables:**

* Identity Service: register, login (JWT), email verification, refresh token.

* Wallet Service: tạo ví, double-entry ledger, balance query, mock top-up.

* API Gateway: routing, JWT validation.

* docker-compose: PostgreSQL, Redis, Kafka, tất cả service.

* React: trang login, đăng ký, dashboard hiển thị balance.

## **8.3 Phase 2 — Transfer**

### **Deliverables:**

* Transfer Service: P2P transfer với Saga choreography qua Kafka.

* Idempotency key \+ outbox pattern hoàn chỉnh.

* Compensating transaction khi fail ở bất kỳ bước nào.

* React: form chuyển tiền, xác nhận OTP mock, lịch sử giao dịch.

## **8.4 Phase 3 — Analytics \+ CQRS \+ Spring Batch**

### **Deliverables:**

* Spring Batch Job config: JobLauncher, Job, Step, ItemReader (JPA), ItemProcessor, ItemWriter (JPA) cho spending aggregation.

* CQRS materialized view: spending\_snapshots được Batch job populate; Kafka consumer update incremental sau mỗi transfer.

* Budget tracking: @Scheduled check \+ Kafka event khi vượt threshold.

* MapStruct mapper cho SpendingSnapshot → AnalyticsDTO.

* Flyway migration cho analytics schema (V3\_\_add\_spending\_snapshots.sql).

* FX Conversion Transfer flow hoàn chỉnh với Saga; rate lock 30 giây.

## **8.5 Phase 4 — C++ FX Rate Engine \+ FX Conversion**

### **Deliverables:**

* C++17 FX Rate Engine: Boost.Asio TCP server \+ mock rate feed simulator.

* CQRS materialized view; query spending dashboard \< 50ms với Spring Data JPA \+ PostgreSQL index.

* Java Kafka consumer (Spring Kafka): consume fx.rates.raw → tính spread → cache Redis → publish fx.rates.processed.

* React: spending charts (recharts), timeline 30/90 ngày, budget vs actual, export CSV.

## **8.6 Phase 5 — Fraud Detection & Notification**

### **Deliverables:**

* Velocity check bằng Redis sliding window counter (5 transfer/phút).

* Unusual amount detection (gấp 10x trung bình 30 ngày).

* New device detection và email confirmation flow.

* Notification Service: WebSocket balance update \+ email notification.

* Admin panel: flagged transaction list \+ resolve workflow.

## **8.7 Phase 6 — DevOps & Observability**

### **Deliverables:**

* Kubernetes manifests cho tất cả service \+ HPA configuration.

* Prometheus scrape config \+ Grafana dashboard template.

* Jaeger distributed tracing tích hợp vào tất cả Java service.

* GitHub Actions CI/CD pipeline: build → test → push → deploy.

* k6 load test script: 500 concurrent users, 10 phút.

## **8.8 Job Readiness Assessment**

| Milestone | Employability Signal |
| ----- | ----- |
| Phase 1–2 hoàn thành | Có thể present microservices \+ Spring Boot \+ Kafka knowledge |
| Phase 3 hoàn thành | Spring Batch \+ CQRS \+ Kafka analytics pipeline — topic hay gặp trong fintech interview; có thể demo analytics dashboard |
| Phase 4–5 hoàn thành | C++ FX Engine \+ fraud detection \+ real-time STOMP notification — điểm khác biệt rõ trong CV; phù hợp senior/specialized fintech role |
| Phase 6 hoàn thành | Portfolio production-ready; phù hợp apply mid-level backend / DevOps position |
